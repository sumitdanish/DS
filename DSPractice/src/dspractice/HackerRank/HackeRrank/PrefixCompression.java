...
public class PrefixCompression {
	...
	try {
	    ...
	    System.out.println(x.length() != 0 ? x.length() - i + " "
		    + x.subSequence(i, x.length()) : 0 + " " + 0);
	    System.out.println(y.length() != 0 ? y.length() - i + " "
		    + y.subSequence(i, y.length()) : 0 + " " + 0);
